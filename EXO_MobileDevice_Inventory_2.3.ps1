...
Import-Csv $devicesOutput | sort-object DisplayName | select DisplayName,DeviceOS,UserAgent,AccessState,AccessReason,FirstSyncTime,LastPolicyUpdate | convertTo-HTML -Title "EXO Mobile Device Report by DisplayName" -CssUri "$scriptpath\simplestyle.css" | out-file -filepath $reportNameHtml -force
